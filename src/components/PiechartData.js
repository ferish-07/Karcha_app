const generatePieChartData = amount => {
  const item = amount;
  console.log(
    '------------amount------------amount-----------',
    item,
    Object.values(converted).length,
  );
  const itemsCount = 3;
  const value = [];
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    data.push({
      percent,
      color:
        Object.values(converted)[i % Object.values(converted).length].color,
    });
  }
  return data;
};
...
export {generatePieChartData, converted};
